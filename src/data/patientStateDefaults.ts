import type { PatientState } from '@/types/simulator';

export const patientStateDefaults: Record<string, PatientState> = {
  hospice_means_giving_up: {
    trust: 30,
    fear: 80,
    understanding: 25,
    readiness: 20,
    confusion: 35,
    resistance: 75,
    medicationFear: 40,
    hospiceMisconception: 85,
    caregiverBurden: 65,
    familyConflict: 25,
    griefIntensity: 70,
    spiritualDistress: 20,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  copd_air_hunger_at_home: {
    trust: 35,
    fear: 90,
    understanding: 15,
    readiness: 25,
    confusion: 70,
    resistance: 40,
    medicationFear: 75,
    hospiceMisconception: 60,
    caregiverBurden: 95,
    familyConflict: 20,
    griefIntensity: 80,
    spiritualDistress: 35,
    perceivedHonesty: 40,
    perceivedCompassion: 35,
  },
  hospice_too_soon: {
    trust: 35,
    fear: 70,
    understanding: 20,
    readiness: 35,
    confusion: 80,
    resistance: 70,
    medicationFear: 35,
    hospiceMisconception: 90,
    caregiverBurden: 65,
    familyConflict: 25,
    griefIntensity: 70,
    spiritualDistress: 30,
    perceivedHonesty: 40,
    perceivedCompassion: 35,
  },
  can_change_minds: {
    trust: 40,
    fear: 65,
    understanding: 15,
    readiness: 40,
    confusion: 85,
    resistance: 65,
    medicationFear: 20,
    hospiceMisconception: 90,
    caregiverBurden: 75,
    familyConflict: 30,
    griefIntensity: 65,
    spiritualDistress: 35,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  pain_management_concern: {
    trust: 25,
    fear: 60,
    understanding: 20,
    readiness: 30,
    confusion: 45,
    resistance: 80,
    medicationFear: 20,
    hospiceMisconception: 50,
    caregiverBurden: 85,
    familyConflict: 15,
    griefIntensity: 75,
    spiritualDistress: 25,
    perceivedHonesty: 35,
    perceivedCompassion: 25,
  },
  medication_refusal: {
    trust: 55,
    fear: 55,
    understanding: 30,
    readiness: 50,
    confusion: 60,
    resistance: 35,
    medicationFear: 40,
    hospiceMisconception: 20,
    caregiverBurden: 90,
    familyConflict: 45,
    griefIntensity: 70,
    spiritualDistress: 30,
    perceivedHonesty: 60,
    perceivedCompassion: 55,
  },
  terminal_dyspnea_follow_up: {
    trust: 50,
    fear: 78,
    understanding: 35,
    readiness: 40,
    confusion: 72,
    resistance: 20,
    medicationFear: 85,
    hospiceMisconception: 25,
    caregiverBurden: 92,
    familyConflict: 10,
    griefIntensity: 88,
    spiritualDistress: 45,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  prognostic_uncertainty: {
    trust: 40,
    fear: 75,
    understanding: 20,
    readiness: 35,
    confusion: 85,
    resistance: 35,
    medicationFear: 15,
    hospiceMisconception: 40,
    caregiverBurden: 70,
    familyConflict: 20,
    griefIntensity: 80,
    spiritualDistress: 45,
    perceivedHonesty: 50,
    perceivedCompassion: 45,
  },
  esrd_comfort_care: {
    trust: 30,
    fear: 85,
    understanding: 20,
    readiness: 20,
    confusion: 60,
    resistance: 80,
    medicationFear: 20,
    hospiceMisconception: 55,
    caregiverBurden: 75,
    familyConflict: 90,
    griefIntensity: 80,
    spiritualDistress: 50,
    perceivedHonesty: 35,
    perceivedCompassion: 30,
  },
  advanced_dementia_grief: {
    trust: 40,
    fear: 70,
    understanding: 20,
    readiness: 30,
    confusion: 55,
    resistance: 60,
    medicationFear: 15,
    hospiceMisconception: 65,
    caregiverBurden: 85,
    familyConflict: 15,
    griefIntensity: 90,
    spiritualDistress: 70,
    perceivedHonesty: 50,
    perceivedCompassion: 45,
  },
  active_dying_recognition: {
    trust: 35,
    fear: 95,
    understanding: 15,
    readiness: 40,
    confusion: 80,
    resistance: 15,
    medicationFear: 60,
    hospiceMisconception: 30,
    caregiverBurden: 90,
    familyConflict: 10,
    griefIntensity: 95,
    spiritualDistress: 60,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  terminal_secretion_distress: {
    trust: 40,
    fear: 92,
    understanding: 10,
    readiness: 30,
    confusion: 85,
    resistance: 20,
    medicationFear: 50,
    hospiceMisconception: 45,
    caregiverBurden: 88,
    familyConflict: 5,
    griefIntensity: 85,
    spiritualDistress: 40,
    perceivedHonesty: 40,
    perceivedCompassion: 35,
  },
  breakthrough_pain_at_home: {
    trust: 55,
    fear: 75,
    understanding: 25,
    readiness: 45,
    confusion: 65,
    resistance: 25,
    medicationFear: 80,
    hospiceMisconception: 30,
    caregiverBurden: 92,
    familyConflict: 10,
    griefIntensity: 75,
    spiritualDistress: 35,
    perceivedHonesty: 50,
    perceivedCompassion: 45,
  },
  advance_directive_conflict: {
    trust: 25,
    fear: 70,
    understanding: 15,
    readiness: 20,
    confusion: 75,
    resistance: 80,
    medicationFear: 10,
    hospiceMisconception: 60,
    caregiverBurden: 65,
    familyConflict: 95,
    griefIntensity: 70,
    spiritualDistress: 40,
    perceivedHonesty: 30,
    perceivedCompassion: 25,
  },
  caregiver_burnout: {
    trust: 45,
    fear: 50,
    understanding: 40,
    readiness: 55,
    confusion: 35,
    resistance: 20,
    medicationFear: 10,
    hospiceMisconception: 20,
    caregiverBurden: 98,
    familyConflict: 15,
    griefIntensity: 85,
    spiritualDistress: 65,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  bereavement_first_call: {
    trust: 50,
    fear: 40,
    understanding: 30,
    readiness: 60,
    confusion: 60,
    resistance: 10,
    medicationFear: 5,
    hospiceMisconception: 15,
    caregiverBurden: 40,
    familyConflict: 5,
    griefIntensity: 95,
    spiritualDistress: 75,
    perceivedHonesty: 60,
    perceivedCompassion: 55,
  },
  patient_direct_prognosis: {
    trust: 60,
    fear: 55,
    understanding: 45,
    readiness: 65,
    confusion: 30,
    resistance: 20,
    medicationFear: 15,
    hospiceMisconception: 20,
    caregiverBurden: 70,
    familyConflict: 40,
    griefIntensity: 75,
    spiritualDistress: 50,
    perceivedHonesty: 55,
    perceivedCompassion: 55,
  },
  family_spiritual_conflict: {
    trust: 20,
    fear: 80,
    understanding: 15,
    readiness: 10,
    confusion: 50,
    resistance: 95,
    medicationFear: 10,
    hospiceMisconception: 70,
    caregiverBurden: 60,
    familyConflict: 85,
    griefIntensity: 75,
    spiritualDistress: 90,
    perceivedHonesty: 30,
    perceivedCompassion: 25,
  },
  icu_transfer_to_hospice: {
    trust: 45,
    fear: 85,
    understanding: 15,
    readiness: 20,
    confusion: 80,
    resistance: 60,
    medicationFear: 30,
    hospiceMisconception: 75,
    caregiverBurden: 80,
    familyConflict: 20,
    griefIntensity: 85,
    spiritualDistress: 45,
    perceivedHonesty: 40,
    perceivedCompassion: 40,
  },
  comfort_kit_first_use: {
    trust: 40,
    fear: 85,
    understanding: 20,
    readiness: 35,
    confusion: 65,
    resistance: 70,
    medicationFear: 90,
    hospiceMisconception: 55,
    caregiverBurden: 88,
    familyConflict: 10,
    griefIntensity: 80,
    spiritualDistress: 30,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  nausea_vomiting_home: {
    trust: 45,
    fear: 88,
    understanding: 20,
    readiness: 30,
    confusion: 75,
    resistance: 35,
    medicationFear: 60,
    hospiceMisconception: 40,
    caregiverBurden: 92,
    familyConflict: 15,
    griefIntensity: 78,
    spiritualDistress: 25,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  dehydration_education: {
    trust: 35,
    fear: 65,
    understanding: 15,
    readiness: 25,
    confusion: 80,
    resistance: 85,
    medicationFear: 20,
    hospiceMisconception: 85,
    caregiverBurden: 70,
    familyConflict: 20,
    griefIntensity: 70,
    spiritualDistress: 35,
    perceivedHonesty: 40,
    perceivedCompassion: 35,
  },
  terminal_agitation_home: {
    trust: 35,
    fear: 92,
    understanding: 15,
    readiness: 30,
    confusion: 78,
    resistance: 30,
    medicationFear: 65,
    hospiceMisconception: 45,
    caregiverBurden: 95,
    familyConflict: 10,
    griefIntensity: 90,
    spiritualDistress: 50,
    perceivedHonesty: 40,
    perceivedCompassion: 38,
  },
  seizure_at_home: {
    trust: 30,
    fear: 98,
    understanding: 10,
    readiness: 20,
    confusion: 88,
    resistance: 20,
    medicationFear: 55,
    hospiceMisconception: 50,
    caregiverBurden: 90,
    familyConflict: 5,
    griefIntensity: 88,
    spiritualDistress: 45,
    perceivedHonesty: 35,
    perceivedCompassion: 30,
  },
  coverage_confusion: {
    trust: 40,
    fear: 55,
    understanding: 15,
    readiness: 45,
    confusion: 85,
    resistance: 60,
    medicationFear: 10,
    hospiceMisconception: 80,
    caregiverBurden: 65,
    familyConflict: 25,
    griefIntensity: 65,
    spiritualDistress: 20,
    perceivedHonesty: 45,
    perceivedCompassion: 40,
  },
  hospice_discharge_fear: {
    trust: 40,
    fear: 60,
    understanding: 20,
    readiness: 40,
    confusion: 75,
    resistance: 65,
    medicationFear: 10,
    hospiceMisconception: 88,
    caregiverBurden: 70,
    familyConflict: 20,
    griefIntensity: 70,
    spiritualDistress: 30,
    perceivedHonesty: 45,
    perceivedCompassion: 42,
  },
  inpatient_hospice_request: {
    trust: 55,
    fear: 60,
    understanding: 35,
    readiness: 65,
    confusion: 50,
    resistance: 25,
    medicationFear: 15,
    hospiceMisconception: 45,
    caregiverBurden: 95,
    familyConflict: 15,
    griefIntensity: 80,
    spiritualDistress: 40,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  treatment_just_failed: {
    trust: 40,
    fear: 75,
    understanding: 20,
    readiness: 30,
    confusion: 70,
    resistance: 55,
    medicationFear: 15,
    hospiceMisconception: 65,
    caregiverBurden: 75,
    familyConflict: 25,
    griefIntensity: 90,
    spiritualDistress: 50,
    perceivedHonesty: 40,
    perceivedCompassion: 38,
  },
  sibling_prognosis_conflict: {
    trust: 30,
    fear: 65,
    understanding: 20,
    readiness: 30,
    confusion: 60,
    resistance: 75,
    medicationFear: 10,
    hospiceMisconception: 70,
    caregiverBurden: 70,
    familyConflict: 90,
    griefIntensity: 75,
    spiritualDistress: 35,
    perceivedHonesty: 35,
    perceivedCompassion: 32,
  },
  children_in_home: {
    trust: 50,
    fear: 60,
    understanding: 35,
    readiness: 45,
    confusion: 60,
    resistance: 30,
    medicationFear: 15,
    hospiceMisconception: 30,
    caregiverBurden: 90,
    familyConflict: 20,
    griefIntensity: 85,
    spiritualDistress: 70,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  financial_caregiver_stress: {
    trust: 50,
    fear: 50,
    understanding: 40,
    readiness: 55,
    confusion: 45,
    resistance: 30,
    medicationFear: 10,
    hospiceMisconception: 25,
    caregiverBurden: 95,
    familyConflict: 20,
    griefIntensity: 75,
    spiritualDistress: 40,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  patient_home_death_wish: {
    trust: 55,
    fear: 45,
    understanding: 45,
    readiness: 60,
    confusion: 55,
    resistance: 25,
    medicationFear: 10,
    hospiceMisconception: 20,
    caregiverBurden: 75,
    familyConflict: 30,
    griefIntensity: 88,
    spiritualDistress: 65,
    perceivedHonesty: 60,
    perceivedCompassion: 55,
  },
  anticipatory_grief_isolation: {
    trust: 50,
    fear: 55,
    understanding: 35,
    readiness: 50,
    confusion: 50,
    resistance: 20,
    medicationFear: 5,
    hospiceMisconception: 20,
    caregiverBurden: 80,
    familyConflict: 10,
    griefIntensity: 92,
    spiritualDistress: 75,
    perceivedHonesty: 55,
    perceivedCompassion: 50,
  },
  opioid_history_pain: {
    trust: 30,
    fear: 70,
    understanding: 25,
    readiness: 35,
    confusion: 50,
    resistance: 75,
    medicationFear: 88,
    hospiceMisconception: 45,
    caregiverBurden: 80,
    familyConflict: 25,
    griefIntensity: 75,
    spiritualDistress: 35,
    perceivedHonesty: 35,
    perceivedCompassion: 30,
  },
};
